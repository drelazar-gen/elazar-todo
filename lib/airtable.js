...
function parseNoteTags(text) {
  ...
  // Optional trailing "(nudge: manual)" or "(nudge: auto every day/week/month/N days)"
  // — written by the delegate popup in the app (see app.js's todo-menu nudge
  // step). If omitted entirely (e.g. someone just types the tag by hand),
  // nudging defaults to manual-only, same as before this feature existed.
  const delegateMatch = str.match(/@todo\s+delegate\s+(?:this\s+)?to\s+([A-Za-z][A-Za-z'-]*(?:\s+[A-Za-z][A-Za-z'-]*){0,2})(?:\s*\(nudge:\s*(manual|auto\s+every\s+(day|week|month|(\d+)\s*days?))\))?/i);
  if (delegateMatch) {
    const name = delegateMatch[1].trim();
    let nudge = { mode: 'manual' };
    if (delegateMatch[2] && /^auto/i.test(delegateMatch[2])) {
      const kind = delegateMatch[3].toLowerCase();
      ...
      else nudge = { mode: 'auto', interval: 'Custom', customDays: parseInt(delegateMatch[4], 10) };
    }
    return { type: 'delegate', name, nudge };
  }
...
async function applyNoteTag(newNoteText, fields) {
  ...
  if (tag.type === 'delegate') {
    ...
    confirm(`Answered: Delegated to ${contact.name} (${nudgeLabel}) — sending them a link now.`);
    return contact;
  }
