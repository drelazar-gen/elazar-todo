...
async function addDelegateMessage(recordId, contactRecordId, contactName, message) {
  ...
  }).catch((e) => ({ ok: false, error: (e && e.message) || String(e) })); // never fail the save because the notification email failed
  console.log('[addDelegateMessage] email result:', JSON.stringify(emailResult)); // TEMP debug — remove once notification delivery is confirmed working
